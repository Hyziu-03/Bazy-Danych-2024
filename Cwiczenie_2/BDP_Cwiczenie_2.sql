...
SELECT matchid, player FROM mecze m
INNER JOIN gole g ON m.id = g.matchid
WHERE teamid = 'POL';
...
FROM mecze JOIN gole ON (id = matchid)
WHERE teamid = 'POL';
...
SELECT player FROM mecze JOIN gole ON (id = matchid)
